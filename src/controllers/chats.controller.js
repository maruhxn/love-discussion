...
export const getAllChats = async (req, res, next) => {
  ...
  const chats = await chatRepository.findAllChatsSQL(roomId);
...
export const createChat = async (req, res, next) => {
  ...
  const ulid = await chatRepository.createChatSQL(req.ip, parsedBody);
...
export const getOneChatById = async (req, res, next) => {
  ...
  const chat = await chatRepository.findOneChatByIdSQL(chatId);
...
export const updateChatById = async (req, res, next) => {
  ...
  await chatRepository.updateChatByIdSQL(chatId, parsedBody);
  return res.status(201).json({
    ok: true,
    msg: "채팅 수정 완료",
  });
...
export const deleteChatById = async (req, res, next) => {
  const { chatId } = req.params;
  const deletedCount = await chatRepository.deleteChatByIdSQL(chatId);
